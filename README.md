...
-- NOVA opção: unlock buttons (true = livres, false = grid) -- MODIFICADO PARA true POR PADRÃO
local unlockButtonsEnabled = true
...
local MOB_INDIVIDUAL_POS_FILE="RXZ_IndividualBtnPos.json"
...
-- Carrega posições individuais
local function loadIndividualPositions()
    if not(isfile and isfile(MOB_INDIVIDUAL_POS_FILE)) then return {} end
    local ok,data=pcall(function() return HS:JSONDecode(readfile(MOB_INDIVIDUAL_POS_FILE)) end)
    if ok and type(data)=="table" then return data end; return {}
end
local function saveIndividualPosition(btnKey, x, y)
    if not writefile then return end
    local data = loadIndividualPositions()
    data[btnKey] = {x=x, y=y}
    pcall(function() writefile(MOB_INDIVIDUAL_POS_FILE, HS:JSONEncode(data)) end)
end
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    pcall(function()
        ...
        local pgui=LP:FindFirstChild("PlayerGui");if pgui then local o=pgui:FindFirstChild(guiName);if o then o:Destroy() end end
    end)
...
local function resetAllSettings()
    ...
    unlockButtonsEnabled=true -- reset para true
...
local function buildMobileButtons()
    ...
    -- Container para os botões (usado apenas quando locked = grid)
    local mbGroup = Instance.new("Frame", mobGui)
    mbGroup.Name = "MobileButtons"
    mbGroup.Size = UDim2.new(0, QS*3 + QG*2 + 20, 0, QS*4 + QG*3 + 20)
    mbGroup.Position = UDim2.new(1, - (QS*3 + QG*2 + 34), 0.5, - (QS*4 + QG*3)/2 - 10)
    mbGroup.BackgroundTransparency = 1
    mbGroup.BorderSizePixel = 0
    mbGroup.Active = true
    mbGroup.ZIndex = 100

    if not unlockButtonsEnabled then
        -- Modo grid: carregar posição do grupo
        ...
        -- Modo livre: grupo invisível com tamanho zero, os botões serão filhos diretos do mobGui
        mbGroup.Size = UDim2.new(0, 0, 0, 0)
        mbGroup.Position = UDim2.new(0, 0, 0, 0)
        mbGroup.BackgroundTransparency = 1
    end

    local individualPositions = loadIndividualPositions()
    ...
    local function makeMobileBtn(label, col, rowN, isToggle, onAction)
        ...
        -- === VARIÁVEIS DE ARRASTO (movidas para antes do clique) ===
        local _dn, _sp, _fp, _li, _wd = false, nil, nil, nil, false

        btn.MouseButton1Click:Connect(function()
            if _wd then return end  -- <-- IMPEDE CLIQUE SE ARRASTOU
            if isToggle then
                isOn = not isOn
        ...
        local moveTarget = unlockButtonsEnabled and frame or mbGroup
        local startPosRef = unlockButtonsEnabled and frame or mbGroup

        btn.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                _dn = true; _wd = false
                _sp = i.Position
                _fp = startPosRef.Position
        ...
        UIS.InputChanged:Connect(function(i)
            if i == _li and _dn and _sp and _fp then
                ...
                if math.abs(dx) > 6 or math.abs(dy) > 6 then
                    _wd = true
                    if unlockButtonsEnabled then
                        local newX = math.max(0, _fp.X.Offset + dx)
                        local newY = math.max(0, _fp.Y.Offset + dy)
                        moveTarget.Position = UDim2.new(0, newX, 0, newY)
                    else
                        moveTarget.Position = UDim2.new(_fp.X.Scale, _fp.X.Offset + dx, _fp.Y.Scale, _fp.Y.Offset + dy)
                    end
                end
            end
        end)
        btn.InputEnded:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                if _wd then
                    if unlockButtonsEnabled then
                        local pos = moveTarget.Position
                        saveIndividualPosition(label, pos.X.Offset, pos.Y.Offset)
                    else
...
pcall(function()
    ...
    if type(d.unlockButtonsEnabled)=="boolean" then unlockButtonsEnabled=d.unlockButtonsEnabled end
end)
...
;(function()
    ...
    -- NOVA OPÇÃO: Unlock Buttons
    addToggleRow(vi,"Unlock Buttons",unlockButtonsEnabled,6,nil,function(on)
        unlockButtonsEnabled=on
        rebuildMobileButtons()
        saveConfig()
    end)
